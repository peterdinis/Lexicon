...
export function updatePage(id: string, updates: Partial<Omit<Page, "id" | "createdAt">>) {
  const pages = read();
...
export function movePageToTrash(id: string) {
  ...
  const idx = pages.findIndex((p) => p.id === id);
  if (idx === -1) return;
  pages[idx] = { ...pages[idx], isTrashed: true, trashedAt: Date.now(), updatedAt: Date.now() };
  write(pages);
}
...
export function restorePage(id: string) {
  ...
  const idx = pages.findIndex((p) => p.id === id);
  if (idx === -1) return;
  pages[idx] = { ...pages[idx], isTrashed: false, trashedAt: undefined, updatedAt: Date.now() };
  write(pages);
}
