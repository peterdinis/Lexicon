...
const AdvancedQuillEditor: React.FC = () => {
  ...
  const modules = {
    ...
    mention: {
      ...
      source: function (searchTerm: string, renderList: (matches: any[], term: string) => void) {
        const users = [{ id: 1, value: 'Alice' }, { id: 2, value: 'Bob' }];
  ...
  const handleTextChange = (
    ...
    editor: { getText: (index: number, length: number) => string; getSelection: () => { index: number; length: number } | null }
  ) => {
    setValue(content);
    if (source === 'user') {
      ...
      if (lastChar === '/') {
        ...
        setSlashPosition({ top: bounds!.top + bounds!.height, left: bounds!.left });
