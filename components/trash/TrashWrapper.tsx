import { Page, listTrashedPages, pagesEvents, restorePage, deletePagePermanent } from "@/store/pageStore";
    ...
    return (
        <DashboardLayout>
            <header className="mb-6">
                ...
                <p className="text-sm text-muted-foreground mt-1">Restore pages or delete them forever.</p>
            </header>
                    ...
                    <CardContent>
                        {pages.length === 0 ? (
                            ...
                            <ul className="space-y-2">
                                {pages.map((p) => (
                                    <li key={p.id} className="flex items-center justify-between rounded-md border px-3 py-2 bg-background/50">
                                        <div className="min-w-0">
                                            <div className="font-medium truncate" title={p.title || "Untitled"}>{p.title || "Untitled"}</div>
                                            <div className="text-xs text-muted-foreground">Trashed {p.trashedAt ? new Date(p.trashedAt).toLocaleString() : ""}</div>
                                        </div>
