...
export const useWorkspaces = () => {
  const { data, loading, error, refetch } = useQuery<{ workspaces: { items: Workspace[] } }>(GET_WORKSPACES);
  const workspaces = data?.workspaces?.items || [];

  const [switchWorkspaceMutation, { loading: switchLoading }] = useMutation(SWITCH_WORKSPACE, {
